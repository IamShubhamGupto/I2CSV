...
parser.add_argument(
    "--num_sheets", type=int, default=5, help="number of sheets to generate"
)

# Add arguments for mean and standard deviation
parser.add_argument("--mean_point", type=float, default=10, help="mean points value")
parser.add_argument(
    "--stddev_point", type=float, default=3, help="standard deviation points value"
)

parser.add_argument("--mean_rebound", type=float, default=3, help="mean rebounds value")
parser.add_argument(
    "--stddev_rebound", type=float, default=1, help="standard deviation rebound value"
)

parser.add_argument("--mean_assist", type=float, default=2, help="mean assists value")
parser.add_argument(
    "--stddev_assist", type=float, default=0.6, help="standard deviation assists value"
)

parser.add_argument("--mean_steal", type=float, default=0.5, help="mean steal value")
parser.add_argument(
    "--stddev_steal", type=float, default=0.1, help="standard deviation steal value"
)

parser.add_argument("--mean_block", type=float, default=0.5, help="mean blocks value")
parser.add_argument(
    "--stddev_block", type=float, default=0.1, help="standard deviation blocks value"
)

parser.add_argument(
    "--mean_turnover", type=float, default=0.5, help="mean blocks value"
)
parser.add_argument(
    "--stddev_turnover", type=float, default=0.1, help="standard deviation blocks value"
)

parser.add_argument("--mean_foul", type=float, default=1, help="mean fouls value")
parser.add_argument(
    "--stddev_foul", type=float, default=0.1, help="standard deviation blocks value"
)
...
if __name__ == "__main__":
    ...
    for sheet in range(args.num_sheets):
        ...
        turnovers = np.random.normal(
            args.mean_turnover, args.stddev_turnover, args.size
        )
        ...
        fig, ax = plt.subplots(
            figsize=(10, 6)
        )  # Adjust the figure size to match your actual data
        ax.axis("off")  # Hide the axes

        # Add a title with varying content and placement
        title = "Simple Basketball Stat Sheet"
        ax.set_title(
            title, fontdict={"fontsize": 14, "fontweight": "bold"}, loc="center"
        )
        ...
        # Save the figure
        plt.savefig(
            f"../data/sheets/sheet_{sheet}_{args.size}_p{args.mean_point}_r{args.mean_rebound}_a{args.mean_assist}_s{args.mean_steal}_b{args.mean_block}_t{args.mean_turnover}_f{args.mean_foul}.png",
            dpi=300,
        )
        plt.close()
    ...
    with open(
        f"../data/pickles/sheet_{args.size}_p{args.mean_point}_r{args.mean_rebound}_a{args.mean_assist}_s{args.mean_steal}_b{args.mean_block}_t{args.mean_turnover}_f{args.mean_foul}.pkl",
        "wb",
    ) as f:
        pickle.dump(all_sheets_data, f)

    logging.debug(
        f"Data was saved to ../data/pickles/sheet_{args.size}_p{args.mean_point}_r{args.mean_rebound}_a{args.mean_assist}_s{args.mean_steal}_b{args.mean_block}_t{args.mean_turnover}_f{args.mean_foul}.pkl"
    )
